...
const NavMobile: React.FC<NavMobileProps> = ({
  ...
  const { connectWallet, walletAddress }: any = useSigningClient();
  ...
  return (
    <div className="overflow-y-auto w-full max-w-sm h-screen py-2 transition transform shadow-lg ring-1 dark:ring-neutral-700 bg-white dark:bg-[#191818] divide-y-2 divide-neutral-100 dark:divide-neutral-800">
      ...
      <div className="flex items-center justify-between py-6 px-5 space-x-2">
        {walletAddress ?
          <ButtonPrimary href={"/createCollection"} className="flex-1">
            Create collection
          </ButtonPrimary> :
          <ButtonSecondary onClick={() => authenticate()} className="flex-1">
            Connect Wallet
          </ButtonSecondary>
        }
      </div>
