...
export default function SortOrderFilter(props: any) {
  const [selected, setSelected] = useState(sortOrder[0]);

  return (
    <div className="">
      <Listbox value={props?.value || selected} onChange={props?.setValue || setSelected}>
        <div className="relative mt-1">
          <Listbox.Button as={Fragment}>
            <button className="font-medium border bg-white dark:bg-[#191818] border-neutral-200 text-neutral-500 dark:text-neutral-400 dark:border-neutral-600 inline-flex items-center justify-center rounded-full px-5 py-2">
              ...
              <span className="block truncate ml-2.5">{props?.value}</span>
              <span className="ml-5">
          ...
          >
            <Listbox.Options className="absolute w-full py-1 mt-2 overflow-auto text-base bg-white rounded-2xl shadow-lg max-h-60 ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm z-50">
              {props?.options && props?.options.map((person: any, personIdx: number) => (
                <Listbox.Option
                  key={personIdx}
                  className={({ active }) =>
