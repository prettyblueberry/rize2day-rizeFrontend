...
const CardNFTMusic2: FC<CardNFTMusic2Props> = ({
  ...
  const renderDefaultBtnListen = (state?: "loading" | "playing") => {
    return (
      <span className="flex items-center justify-center w-10 h-10 bg-white rounded-full shadow-lg cursor-pointer sm:w-11 sm:h-11 dark:bg-[#191818]/50 text-primary-6000 dark:text-primary-200">
        {renderIcon(state)}
      </span>
    );
  };
  ...
  return (
    <div
      className={`nc-CardNFTMusic2 z-10 relative flex justify-between p-2 space-x-2 rounded-3xl bg-neutral-100 dark:bg-neutral-800 hover:shadow-xl transition-shadow ${className}`}
      data-nc-id="CardNFTMusic2"
    >
      <Link to={"/nft-detail"} className="flex flex-grow space-x-4">
